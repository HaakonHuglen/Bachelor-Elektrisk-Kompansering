...
csvFileName = 'MainHardUtenTC3PID.csv';
tSearchStart = 0;        % [s] skip first sinus cycle and use the next full cycle
saveFigures  = false;     % true = save figures as PNG
...
if ~isfile(csvFile)
    % Fallback: allow running from a parent project folder.
...
% This file does not include a directly logged total U signal.
% Therefore, the total command is reconstructed from the two contributions.
uTotal = uFF + uPID;
...
uTotalPlot = uTotal(idx);
uFFPlot    = uFF(idx);
uPIDPlot   = uPID(idx);

fprintf('\n============================================================\n');
fprintf('TEST 07 - MAIN HARD LOAD WITHOUT TC3 PID\n');
...
figure('Name','Test 07 - Position Tracking','Color','w');
plot(tPlot, xRefPlot, 'LineWidth', 1.8); hold on;
plot(tPlot, xRealPlot, '', 'LineWidth', 1.8);
grid on;
box on;
xlabel('Time [s]');
ylabel('Position [mm]');
title('Test 7 - Main hard load without TC3 PID');
legend('$x_{ref}$', '$x_{real}$', 'Interpreter','latex', 'Location','best');

if saveFigures
    exportgraphics(gcf, fullfile(scriptFolder, 'Test07_PositionTracking.png'), 'Resolution', 300);
end

%% ===================== FIGURE 2: CONTROL SIGNAL =====================

figure('Name','Test 07 - Control Signal','Color','w');
plot(tPlot, uTotalPlot, 'LineWidth', 1.8); hold on;
...
title('Test 7 - Control signal without TC3 PID');
legend('$U_{total}$', '$U_{FF}$', '$U_{PID}$', 'Interpreter','latex', 'Location','best');

if saveFigures
    exportgraphics(gcf, fullfile(scriptFolder, 'Test07_ControlSignal.png'), 'Resolution', 300);
end

%% ===================== LOCAL FUNCTIONS =====================

function D = readTwinCATScopeCsv(filePath)
    % Reads TwinCAT Scope CSV files where each signal is stored as:
    ...
    for i = headerIdx+2:numel(lines)
        ...
        tValue = str2double(strrep(parts(1), ',', '.')) / 1000; % ms -> s
        if isnan(tValue)
            continue;
        end
