%% ============================================================
%  TEST 08 - MAIN HARD LOAD WITHOUT DMA PID
%
...
% This file includes fU. It is used as the logged total/reference valve command.
% If fU is missing in another file, the script reconstructs U_total from U_FF + U_PID.
if any(strcmp(D.Properties.VariableNames, 'fU'))
    uTotal = D.fU;
else
    uTotal = uFF + uPID;
end
...
fprintf('\n============================================================\n');
fprintf('TEST 08 - MAIN HARD LOAD WITHOUT DMA PID\n');
...
figure('Name','Test 08 - Position Tracking','Color','w');
...
xlabel('Time [s]');
ylabel('Position [mm]');
title('Test 8 - Main hard load without DMA PID');
legend('$x_{ref}$', '$x_{real}$', 'Interpreter','latex', 'Location','best');

if saveFigures
    exportgraphics(gcf, fullfile(scriptFolder, 'Test08_PositionTracking.png'), 'Resolution', 300);
end

%% ===================== FIGURE 2: CONTROL SIGNAL =====================

figure('Name','Test 08 - Control Signal','Color','w');
...
title('Test 8 - Control signal without DMA PID');
legend('$U_{total}$', '$U_{FF}$', '$U_{PID}$', 'Interpreter','latex', 'Location','best');

if saveFigures
    exportgraphics(gcf, fullfile(scriptFolder, 'Test08_ControlSignal.png'), 'Resolution', 300);
end
