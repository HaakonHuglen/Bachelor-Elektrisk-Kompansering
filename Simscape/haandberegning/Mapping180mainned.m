...
folderPath = 'C:\Users\Oscar\Desktop\Bachelor-Elektrisk-Kompansering\Matlab';
...
% ===================== FIGUR 5: Kv vs u =====================
Kv_mean = Q_mean ./ sqrt(dp_mean * 1e5);          % [m^3/(s·√Pa)]
Kv_mean = Kv_mean * 60000 * sqrt(1e5);             % [L/(min·√bar)]

figure;

valid_kv    = ~isnan(u_percent) & ~isnan(Kv_mean) & Kv_mean > 0;
deadband_kv = ~isnan(u_percent) & Kv_mean == 0;

u_fit_kv = u_percent(valid_kv);
Kv_fit   = Kv_mean(valid_kv);

plot(u_percent(valid_kv), Kv_mean(valid_kv), 'o', 'MarkerSize', 7, 'LineWidth', 1.5);
hold on;

if any(deadband_kv)
    plot(u_percent(deadband_kv), zeros(sum(deadband_kv),1), 'rx', ...
        'MarkerSize', 10, 'LineWidth', 2);
end

polyDegree = 3;
if numel(u_fit_kv) >= polyDegree + 1
    pKv = polyfit(u_fit_kv, Kv_fit, polyDegree);
    u_dense_kv = linspace(min(u_fit_kv), max(u_fit_kv), 300);
    Kv_dense   = polyval(pKv, u_dense_kv);
    plot(u_dense_kv, Kv_dense, '-', 'LineWidth', 1.8);

    if any(deadband_kv) && any(valid_kv)
        u_db_grense = max(u_percent(deadband_kv));
        ymax = max(Kv_mean(valid_kv));
        xline(u_db_grense, '--k', 'LineWidth', 1.2);
        text(u_db_grense + 0.5, ymax * 0.5, 'Dødbånd', ...
            'FontSize', 10, 'Color', 'k');
    end

    legend('Målepunkter', 'Dødbånd (K_v = 0)', ...
           sprintf('Polynomfit grad %d', polyDegree), 'Location', 'best');
    disp('Polyfit-koeffisienter for K_v(u):');
    disp(pKv);
else
    legend('Målepunkter', 'Dødbånd (K_v = 0)', 'Location', 'best');
end

grid on;
xlabel('u [%]');
ylabel('$K_v \left[\frac{L}{\mathrm{min}\cdot\sqrt{\mathrm{bar}}}\right]$', ...
       'Interpreter', 'latex')
%ylabel('K_v [\frac{L}{(min \cdot \surdbar}]');
title('K_v');