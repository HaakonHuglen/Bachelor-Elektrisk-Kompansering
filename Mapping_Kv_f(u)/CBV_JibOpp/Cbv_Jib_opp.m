%% ========================================================================
%  JIB OPP CBV IDENTIFICATION - P TO A, B-SIDE CBV
%
%  Purpose:
%  Identify effective CBV parameters for Jib opp.
%
%  For Jib opp, the PDCV is assumed to connect:
%
%       P -> A
%       B -> T
%
%  Therefore, the B-side CBV is assumed to be active.
%
%  Opening pressure:
%
%       p_open,Jib,B = alpha*pA + pB_cyl - (alpha + 1)*pB
%
%  where:
%
%       pA     = fPres1A_bar
%       pB     = fPres1B_bar
%       pB_cyl = fPresCyl1B_bar
%
% ========================================================================
...
folderPath = 'C:\Users\hakon\OneDrive - Universitetet i Agder\Skrivebord\BACHELOR_PROSJEKT\Bachelor-Oppgave---Elektrisk-Kompansering\Mapping_Kv_f(u)\CBV_JibOpp';
...
% Jib opp in these files gives increasing cylinder position.
...
% Check Figure 3 after running and adjust if needed.
steadyWindows = [
    4.1   20.0;    % OppPOPEN40
    4.1   20.0;    % OppPOPEN70
    4.1   20.0     % OppPOPEN90
...
% Choose whether final pCr uses mean or median.
% "mean" is consistent with repeated tests.
% "median" is more robust if one test deviates.
pCrMethod = "mean";   % "mean" or "median"

showTimeSeriesFigure = true;

saveFigures = false;

figureFolder = fullfile(folderPath, 'CBV_Jib_Opp_Final_Figures');
...
uMotion_crack_signed_percent = nan(nCrack, 1);
uMotion_crack_abs_percent = nan(nCrack, 1);
...
for i = 1:nCrack
    ...
    % Signed opening convention:
    % 5 V = 0%.
    % For Jib opp these tests go in the negative direction.
    uValve_signed_percent = (5 - uValve_V) / 5 * 100;

    % Absolute opening magnitude used for easier interpretation in tables.
    uValve_abs_percent = abs(uValve_signed_percent);

    % Jib opp, B-side CBV opening pressure.
    pOpen = alpha .* pA + pB_cyl - (alpha + 1) .* pB;

    % A-side expression, only for checking.
    pOpen_A_side_check = alpha .* pB + pA_cyl - (alpha + 1) .* pA;
    ...
    if motionDirection == "increasing"

        motionIdx = find((xFilt - x0) >= motionThreshold_mm, 1, 'first');

    elseif motionDirection == "decreasing"

        motionIdx = find((x0 - xFilt) >= motionThreshold_mm, 1, 'first');

    else

        error('motionDirection must be "increasing" or "decreasing".');
    ...
    uMotion_crack_signed_percent(i) = uValve_signed_percent(motionIdx);
    uMotion_crack_abs_percent(i) = uValve_abs_percent(motionIdx);
    ...
    crackData{i}.pOpen_A_side_check = pOpen_A_side_check;
    crackData{i}.x = x;
    crackData{i}.xFilt = xFilt;
    crackData{i}.v = v;
    crackData{i}.uValve_signed_percent = uValve_signed_percent;
    crackData{i}.uValve_abs_percent = uValve_abs_percent;
...
if pCrMethod == "mean"
    pCr_Jib_B_bar = pCr_mean_bar;
elseif pCrMethod == "median"
    pCr_Jib_B_bar = pCr_median_bar;
else
    error('pCrMethod must be "mean" or "median".');
end

pCr_Jib_B_Pa = pCr_Jib_B_bar * 1e5;
...
uMotion_pOpen_signed_percent = nan(nPOpen, 1);
uMotion_pOpen_abs_percent = nan(nPOpen, 1);
...
for i = 1:nPOpen
    ...
    if ~isempty(motionIdx)
        ...
        uMotion_pOpen_signed_percent(i) = uValve_signed_percent(motionIdx);
        uMotion_pOpen_abs_percent(i) = uValve_abs_percent(motionIdx);
    ...
    pOpenData{i}.pOpen_A_side_check = pOpen_A_side_check;
    pOpenData{i}.x = x;
    pOpenData{i}.xFilt = xFilt;
    pOpenData{i}.v = v;
    pOpenData{i}.uValve_signed_percent = uValve_signed_percent;
    pOpenData{i}.uValve_abs_percent = uValve_abs_percent;
...
dpOpen_Jib_B_bar = pOpenHigh_bar - pCr_Jib_B_bar;

dpOpen_Jib_B_Pa = dpOpen_Jib_B_bar * 1e5;
...
crackResults = table( ...
    crackLabels(:), ...
    tMotion_crack_s, ...
    uMotion_crack_signed_percent, ...
    uMotion_crack_abs_percent, ...
    pA_motion_bar, ...
    pB_motion_bar, ...
    pA_cyl_motion_bar, ...
    pB_cyl_motion_bar, ...
    ...
    'VariableNames', { ...
        'Test', ...
        't_motion_start_s', ...
        'u_signed_at_motion_start_percent', ...
        'u_abs_at_motion_start_percent', ...
        'pA_at_motion_bar', ...
        'pB_at_motion_bar', ...
...
pOpenResults = table( ...
    ...
    pOpenStable_bar, ...
    vStable_mmps, ...
    pOpenStable_bar - pCr_Jib_B_bar, ...
    tMotion_pOpen_s, ...
    uMotion_pOpen_signed_percent, ...
    uMotion_pOpen_abs_percent, ...
    ...
    'VariableNames', { ...
        ...
        'u_signed_at_motion_start_percent_for_fun', ...
        'u_abs_at_motion_start_percent_for_fun', ...
...
disp(' ');
disp('============================================================');
disp('JIB OPP B-SIDE CBV CRACK PRESSURE RESULTS');
disp('============================================================');
disp(crackResults);

disp(' ');
disp('============================================================');
disp('JIB OPP B-SIDE CBV OPENING RANGE RESULTS');
disp('============================================================');
disp(pOpenResults);

fprintf('\nFinal Jib Opp B-side CBV parameters, alpha = %.2f:\n', alpha);
fprintf('pCr_Jib_B mean      = %.2f bar\n', pCr_mean_bar);
fprintf('pCr_Jib_B median    = %.2f bar\n', pCr_median_bar);
fprintf('pCr_Jib_B used      = %.2f bar = %.3e Pa\n', pCr_Jib_B_bar, pCr_Jib_B_Pa);
fprintf('pOpen_high used     = %.2f bar\n', pOpenHigh_bar);
fprintf('dpOpen_Jib_B        = %.2f bar = %.3e Pa\n', dpOpen_Jib_B_bar, dpOpen_Jib_B_Pa);

fprintf('\nRounded values I would test first in Simscape:\n');
fprintf('pCr_Jib_B = %.0fe5;        %% [Pa] = %.0f bar\n', round(pCr_Jib_B_bar), round(pCr_Jib_B_bar));
fprintf('dpOpen_Jib_B = %.0fe5;     %% [Pa] = %.0f bar\n', round(dpOpen_Jib_B_bar), round(dpOpen_Jib_B_bar));

fprintf('\nExact copy to Simscape/MATLAB parameter file:\n');
fprintf('pCr_Jib_B = %.6g;        %% [Pa]\n', pCr_Jib_B_Pa);
fprintf('dpOpen_Jib_B = %.6g;     %% [Pa]\n', dpOpen_Jib_B_Pa);

%% ========================================================================
%  FIGURE 1: CRACK PRESSURE IDENTIFICATION
% ========================================================================

figure('Name', 'Figure 1 - Jib Opp B-side CBV Crack Pressure');
hold on;
grid on;
box on;
...
yline(pCr_Jib_B_bar, '--', ...
    sprintf('p_{cr,Jib,B} = %.1f bar', pCr_Jib_B_bar), ...
    'LineWidth', 1.4);

xlabel('Time [s]');
ylabel('Opening pressure, p_{open,Jib,B} [bar]');
title('Jib Opp B-side CBV Crack Pressure Identification');
...
figure('Name', 'Figure 2 - Jib Opp B-side CBV Opening Range');
hold on;
grid on;
box on;
...
text(65, (pCr_Jib_B_bar + pOpenHigh_bar)/2, ...
    sprintf('\\Delta p_{open,Jib,B} = %.1f bar', dpOpen_Jib_B_bar), ...
    'FontSize', 11, ...
    'FontWeight', 'bold', ...
    'HorizontalAlignment', 'center');

xlabel('Nominal PDCV opening [%]');
ylabel('Steady-state opening pressure, p_{open,Jib,B} [bar]');
title('Jib Opp B-side CBV Opening Range Identification');
...
if showTimeSeriesFigure
    ...
    figure('Name', 'Figure 3 - Jib Opp B-side CBV pOpen Time Series');
    hold on;
    grid on;
    box on;
    ...
    xlabel('Time [s]');
    ylabel('Opening pressure, p_{open,Jib,B} [bar]');
    title('Jib Opp B-side CBV Opening Pressure with Selected Intervals');
