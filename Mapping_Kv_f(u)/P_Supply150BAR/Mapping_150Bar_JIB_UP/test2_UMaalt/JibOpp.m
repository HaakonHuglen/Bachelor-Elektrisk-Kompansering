...
%% ===================== LES MAPPER =====================
T_main  = lesMappeJibOpp(folderPathMain,  false, activeSide, A_piston, A_annulus, Cd, rho, velThreshold);
T_extra = lesMappeJibOpp(folderPathExtra, true,  activeSide, A_piston, A_annulus, Cd, rho, velThreshold);
...
%% ===================== FIGUR 4: Ad vs u =====================
plotMedFitJibOpp(u, Ad, ...
    'A_d [m^2]', ...
    'Effektivt orifice-areal vs u - Jib opp', ...
    'A_d');

%% ===================== FIGUR 5: Kv vs u =====================
plotMedFitJibOpp(u, Kv, ...
...
if any(mask)
    plotMedFitJibOpp(u(mask), Kv(mask), ...
        'K_v [L/(min \cdot \surdbar)]', ...
        'K_v vs u - zoom dødbånd - Jib opp', ...
        'K_v');
else
    warning('Ingen punkter funnet i valgt zoom-område for dødbånd.');
end

%% ===================== PLOT MED FIT JIB OPP =====================
function plotMedFitJibOpp(u, y, ylbl, ttl, varName)
...
%% ===================== LES MAPPE JIB OPP =====================
function T = lesMappeJibOpp(folderPath, isDeadbandFolder, activeSide, A_piston, A_annulus, Cd, rho, velThreshold)
    ...
    for k = 1:nFiles
        ...
        if ~isempty(headerIdx)
            ...
            idx_sen = find(signalNames == "fSensor1_V",      1);
            idx_val = find(signalNames == "fValve1_V",       1);
            ...
            if ~isempty(idx_ps) && ~isempty(idx_pa) && ~isempty(idx_pb) && ...
               ~isempty(idx_sen) && ~isempty(idx_x)
                ...
                for i = headerIdx+1:numel(rawLines)
                    ...
                    uSen(end+1,1) = valVals(idx_sen);

                    if ~isempty(idx_val)
                        uRef(end+1,1) = valVals(idx_val);
                    else
                        uRef(end+1,1) = NaN;
                    end
                end
        ...
        % Fallback for hovedmappeformat med fast kolonnerekkefølge.
        if numel(t_ms) < 5
