...
folderPath = 'C:\Users\hakon\OneDrive - Universitetet i Agder\Skrivebord\BACHELOR_PROSJEKT\Bachelor-Oppgave---Elektrisk-Kompansering\Mapping_Kv_f(u)\P_Supply150BAR\Mapping_150Bar_JIB_UP\Test2_UMaalt';
...
for k = 1:nFiles
    ...
    % Bruk bare rad 10 til EOF
    if numel(rawLines) < 10
        warning('Fil %s har færre enn 10 linjer. Hopper over.', files(k).name);
        continue;
    end

    dataLines = rawLines(10:end);
    ...
    % Forventet volt fra prosent: 0% -> 5V, 100% -> 10V
    ...
    for i = 1:numel(dataLines)
        ...
        % Forventer format:
        % t ; ps ; t ; pa ; t ; pb ; t ; x ; t ; u
        if numel(parts) < 10
            continue;
        end

        vals = str2double(parts(1:10));
        if any(isnan(vals))
            continue;
        end
        ...
        % ===== HER ER ENDRINGEN: pa og pb byttes =====
        pa(end+1,1)   = vals(6);
        pb(end+1,1)   = vals(4);
...
%% ===================== EXPORT =====================
writetable(results, fullfile(folderPath, 'results_manual_cleaned_from_row10.csv'));

%% ===================== PLOTS =====================
figure;
plot(u_percent, uV_exp, '-o', 'LineWidth', 1.5);
hold on;
plot(u_percent, uV_mean, '-o', 'LineWidth', 1.5);
grid on;
xlabel('u [%]');
ylabel('U [V]');
legend('Expected U','Measured U','Location','best');
title('Measured U vs expected U');
