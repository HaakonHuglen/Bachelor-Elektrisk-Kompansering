...
%% ===================== INNSTILLINGER =====================
folderPathMain  = 'C:\Users\hakon\OneDrive - Universitetet i Agder\Skrivebord\BACHELOR_PROSJEKT\Bachelor-Oppgave---Elektrisk-Kompansering\Mapping_Kv_f(u)\P_Supply150BAR\Mapping_150Bar_MAIN_UP\MainOpp150bar';
folderPathExtra = 'C:\Users\hakon\OneDrive - Universitetet i Agder\Skrivebord\BACHELOR_PROSJEKT\Bachelor-Oppgave---Elektrisk-Kompansering\Mapping_Kv_f(u)\P_Supply150BAR\Mapping_150Bar_MAIN_UP\dødbånd';
...
T_extra = lesMappeMainOpp(folderPathExtra, true,  activeSide, A_piston, A_annulus, Cd, rho, velThreshold);
...
if any(mask)
    plotMedFitMainOpp(u(mask), Kv(mask), ...
        'K_v [L/(min \cdot \surdbar)]', ...
        'K_v vs u - zoom dødbånd - Main opp', ...
        'K_v');
else
...
function plotMedFitMainOpp(u, y, ylbl, ttl, varName)
    ...
    % Ikke bruk overgangsområde nær dødbånd i fit.
    fitMask = valid & u >= 23.4;
    ...
    if any(deadband)
        u_db = max(u(deadband));   % riktig for positiv retning
        xline(u_db, '--k', 'LineWidth', 1.2);

        if any(valid)
            text(u_db + 0.3, max(y(valid))*0.5, 'Dødbånd', 'FontSize', 10);
        end
    end
...
function T = lesMappeMainOpp(folderPath, isDeadbandFolder, activeSide, A_piston, A_annulus, Cd, rho, velThreshold)
    ...
    for k = 1:nFiles
        ...
        % Robust hastighet fra lineær fit av posisjon
        p_x = polyfit(t, x, 1);
        xdot_mean(k) = p_x(1);
