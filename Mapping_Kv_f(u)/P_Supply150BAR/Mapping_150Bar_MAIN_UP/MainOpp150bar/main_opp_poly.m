...
%% ===================== SETTINGS =====================
resultsFile = 'C:\Users\hakon\OneDrive - Universitetet i Agder\Skrivebord\BACHELOR_PROSJEKT\Bachelor-Oppgave---Elektrisk-Kompansering\Mapping_Kv_f(u)\P_Supply150BAR\Mapping_150Bar_MAIN_UP\MainOpp150bar\results_combined_main_up.csv';
...
% Main opp: direkte positivt TwinCAT-signal
...
%% ===================== PRINT RESULT =====================
fprintf('\n====================================================\n');
fprintf('TwinCAT-polynom: MAIN OPP 150 bar\n');
fprintf('====================================================\n');
...
ylabel('u_{ctrl} [-]');
title('TwinCAT-polynom: direkte u_{FF} = f(K_v) - Main opp 150 bar');
...
%% ===================== OTHER GROUP: CYLINDER DOWN POLYNOMIAL =====================
% Fra den andre bachelorgruppen:
% fSpoolPos(Kv) =
% +0.00098272*Kv^6
% -0.017507*Kv^5
% +0.11599*Kv^4
...
u_other_down_spool = polyval(p_other_down, Kv_dense);

% Deres ekstra spool/reference-korreksjon:
% fSpoolPosCtrl := (fSpoolPos + 0.0416) / 1.1075;
u_other_down_ctrl = (u_other_down_spool + 0.0416) / 1.1075;

% I deres TwinCAT-kode blir sylinder ned positiv:
% fU_FF := LIMIT(-1.0, fSpoolPosCtrl, 1.0)
u_other_down_ctrl = max(-1.0, min(1.0, u_other_down_ctrl));

plot(Kv_dense, u_other_down_ctrl, '--', 'LineWidth', 1.8);
legend('Målepunkter egne data', ...
    sprintf('Eget polynom grad %d | R^2 = %.4f', polyDeg, R2), ...
    'Andre gruppen: ned-polynom', ...
    'Location', 'best');