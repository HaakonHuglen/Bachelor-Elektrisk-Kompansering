...
%% ===================== INNSTILLINGER =====================
folderPathMain  = 'C:\Users\hakon\OneDrive - Universitetet i Agder\Skrivebord\BACHELOR_PROSJEKT\Bachelor-Oppgave---Elektrisk-Kompansering\Mapping_Kv_f(u)\P_Supply150BAR\Mapping_150Bar_JIB_DOWN\JibNed150bar';
folderPathExtra = 'C:\Users\hakon\OneDrive - Universitetet i Agder\Skrivebord\BACHELOR_PROSJEKT\Bachelor-Oppgave---Elektrisk-Kompansering\Mapping_Kv_f(u)\P_Supply150BAR\Mapping_150Bar_JIB_DOWN\dødbånd';
...
activeSide   = 'B';     % Jib ned -> P til B
Cd           = 0.64;
rho          = 850;
velThreshold = 1e-5;

%% ===================== LES MAPPER =====================
T_main  = lesMappeJibNed(folderPathMain,  false, activeSide, A_piston, A_annulus, Cd, rho, velThreshold);
T_extra = lesMappeJibNed(folderPathExtra, true,  activeSide, A_piston, A_annulus, Cd, rho, velThreshold);
...
disp(results);
writetable(results, fullfile(folderPathMain, 'results_combined_jib_down.csv'));
...
title('Referansesignal vs målt spoleposisjon - Jib ned');
...
title('Middeltrykksverdier vs u - Jib ned');
...
xlabel('u [%]');
ylabel('Q [L/min]');
title('Estimert flow vs u - Jib ned');

%% ===================== FIGUR 4: Ad vs u =====================
plotMedFitJibNed(u, Ad, ...
    'A_d [m^2]', ...
    'Effektivt orifice-areal vs u - Jib ned', ...
    'A_d');

%% ===================== FIGUR 5: Kv vs u =====================
plotMedFitJibNed(u, Kv, ...
...
if any(mask)
    plotMedFitJibNed(u(mask), Kv(mask), ...
        'K_v [L/(min \cdot \surdbar)]', ...
        'K_v vs u - zoom dødbånd - Jib ned', ...
        'K_v');
else
    warning('Ingen punkter funnet i valgt zoom-område for dødbånd.');
end

%% ===================== PLOT MED FIT JIB NED =====================
function plotMedFitJibNed(u, y, ylbl, ttl, varName)
    ...
    % Ikke bruk overgangsområdet nær dødbånd i fit.
...
%% ===================== LES MAPPE JIB NED =====================
function T = lesMappeJibNed(folderPath, isDeadbandFolder, activeSide, A_piston, A_annulus, Cd, rho, velThreshold)
    ...
    for k = 1:nFiles
        ...
        for i = 1:numel(rawLines)
            ...
            if numel(vals) >= 12
                % Format:
                % t ; ps ; t ; pA ; t ; pB ; t ; x ; t ; valve ; t ; sensor
                pa(end+1,1)   = vals(4);
                pb(end+1,1)   = vals(6);
        ...
        % Jib ned: -100 % -> 0 V, 0 % -> 5 V hvis U_ref mangler
        if all(isnan(uRef))
